...
import { resolveQualityInputKind, SUBJECTIVE_SCALE_OPTIONS } from './qualityInput'
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <label className="flex flex-col gap-1 text-xs text-gray-600">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    onChangeField(item.material_id, 'amountCompleted', e.target.value)
                  }
                />
              </label>
              <label className="flex flex-col gap-1 text-xs text-gray-600">
              ...
              {qualityKind !== 'HIDDEN' && (
                <label className="flex flex-col gap-1 text-xs text-gray-600">
                  {t('dailyReport.studyLog.qualityLabel')}
                  {qualityKind === 'PERCENT' ? (
                    <Input
                      type="number"
                      min={0}
                      ...
                      onChange={(e) =>
                        onChangeField(item.material_id, 'qualityValue', e.target.value)
                      }
                    />
                  ) : (
                    <select
