...
from app.schemas.work_evaluation import (
...
@router.post(
    "/goals/{goal_id}/work-assignment/evaluation-reports", response_model=WorkEvaluationReportRead
)
def generate_evaluation_report(
    goal_id: int, payload: WorkEvaluationReportGenerateRequest, session: Session = Depends(get_db)
) -> WorkEvaluationReportRead:
    goal = goal_service.get_goal(session, goal_id)
...
@router.get(
    "/goals/{goal_id}/work-assignment/evaluation-reports",
    response_model=list[WorkEvaluationReportRead],
)
def list_evaluation_reports(
    goal_id: int, member_id: int | None = None, session: Session = Depends(get_db)
    ...
    return [serialize_evaluation_report(report) for report in reports]


@router.patch(
    "/goals/{goal_id}/work-assignment/evaluation-reports/{report_id}",
    response_model=WorkEvaluationReportRead,
)
def update_evaluation_report(
    ...
    goal_service.get_goal(session, goal_id)
    report = work_evaluation_service.get_evaluation_report(session, report_id)
    work_evaluation_service.update_evaluation_report(session, report, body=payload.body)
    session.commit()
    return serialize_evaluation_report(report)
