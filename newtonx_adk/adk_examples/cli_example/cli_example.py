...
try:
    ...
    print(
        "ADKが正しくインストールされているか、srcディレクトリが存在するか確認してください。"
    )
    sys.exit(1)
...
"""
    ...
    def _authenticate(self) -> bool:
        ...
        try:
            ...
            if not config.client_id or not config.tenant_id:
                ...
                self.console.print(
                    "[yellow]以下のコマンドで認証設定を行ってください:[/yellow]"
                )
            ...
            try:
                ...
                web_base = (
                    config.api_base_url or "https://seraku.newton-x.net/api"
                ).rsplit("/api", 1)[0]
                sub = config.company_subdomain or ""
                login_url = f"{web_base}/account/redirectadlogin" + (
                    f"?subdomain={sub}" if sub else ""
                )
                webbrowser.open(login_url)
                self.console.print(
                    f"[cyan]NewtonXログインページを開きました: {login_url}[/cyan]"
                )
            except Exception:
                pass
            ...
            if not internal_token:
                self.console.print(
                    "[red]内部トークンが抽出できませんでした。操作をやり直してください。[/red]"
                )
                return False
            ...
            self.console.print(
                "[cyan]内部トークンを保存しました。セッションをブートストラップします...[/cyan]"
            )
            ok = self.auth_manager.bootstrap_session()
            if not ok:
                self.console.print(
                    "[yellow]ブートストラップに失敗しましたが、続行します。必要に応じて再試行されます。[/yellow]"
                )
    ...
    def _create_chat(self):
        ...
        try:
            ...
            choice = Prompt.ask(
                "選択", choices=[str(i) for i in range(1, len(assistants) + 1)]
            )
            ...
            chat_uid = self.client.create_chat(
                assistant_uid=selected_assistant["uid"], title=title
            )
    ...
    def _send_message(self):
        ...
        try:
            ...
            choice = Prompt.ask(
                "選択", choices=[str(i) for i in range(1, len(chats) + 1)]
            )
            selected_chat = chats[int(choice) - 1]

            # チャットUIDを取得（複数の可能性を試す）
            chat_uid = (
                selected_chat.get("id")
                or selected_chat.get("uid")
                or selected_chat.get("chat_uid")
            )
            ...
            web_search = (
                Prompt.ask(
                    "ウェブ検索を使用しますか？", choices=["y", "n"], default="y"
                )
                == "y"
            )
            ...
            knowledge_search = (
                Prompt.ask(
                    "ナレッジ検索（アップロードしたファイルを参照）を使用しますか？",
                    choices=["y", "n"],
                    default=knowledge_search_default,
                )
                == "y"
            )
            ...
            try:
                ...
                if (
                    chat_detail
                    and "messages" in chat_detail
                    and chat_detail["messages"]
                ):
    ...
    def _send_voice_message(self):
        ...
        try:
            ...
            self.console.print(
                "[yellow]ヒント: タブキーでファイル名を補完できます[/yellow]"
            )
            self.console.print(
                "[yellow]対応形式: .wav, .mp3, .aiff, .aac, .ogg, .flac[/yellow]"
            )
            ...
            if not file_path_input.strip():
                self.console.print(
                    "[yellow]ファイルパスが入力されませんでした。キャンセルします。[/yellow]"
                )
                return
            ...
            if not os.path.exists(file_path):
                ...
                self.console.print(
                    f"[yellow]現在のディレクトリ: {os.getcwd()}[/yellow]"
                )
                return
            ...
            if file_ext not in allowed_extensions:
                ...
                self.console.print(
                    f"[yellow]対応形式: {', '.join(allowed_extensions)}[/yellow]"
                )
                return
            ...
            if file_size > max_size:
                self.console.print(
                    f"[red]ファイルサイズが大きすぎます（15MB以下にしてください）。[/red]"
                )
                self.console.print(
                    f"[yellow]現在のサイズ: {file_size / 1024 / 1024:.2f} MB[/yellow]"
                )
                return
    ...
    def _upload_file(self):
        ...
        try:
            ...
            self.console.print(
                "[yellow]例: ./test_upload.txt または ../documents/file.pdf[/yellow]"
            )
            ...
            if file_size > 50 * 1024 * 1024:  # 50MB制限
                self.console.print(
                    "[red]ファイルサイズが大きすぎます（50MB以下にしてください）。[/red]"
                )
                return
            ...
            if is_image:
                ...
                try:
                    image_id = self.client.upload_image(
                        chat_uid=chat_uid, file_path=file_path
                    )
                    if image_id:
                        self.console.print(
                            f"[green]画像がアップロードされました！[/green]"
                        )
                        self.console.print(f"画像ID: {image_id}")
                    else:
                        self.console.print(
                            "[red]画像のアップロードに失敗しました。[/red]"
                        )
                except Exception as e:
                ...
                try:
                    ...
                    if success:
                        self.console.print(
                            "[green]ドキュメントがアップロードされました！[/green]"
                        )
                    else:
                        self.console.print(
                            "[red]ドキュメントのアップロードに失敗しました。[/red]"
                        )
                except Exception as e:
                    self.console.print(
                        f"[red]ドキュメントアップロードエラー: {e}[/red]"
                    )
        ...
        try:
            ...
            try:
                ...
                if file_uid:
                    ...
                    self.console.print(
                        "[yellow]APIからファイルIDが返らない環境です。削除時はIDの指定が必要です。[/yellow]"
                    )
            except Exception as e:
    ...
    def _delete_assistant_knowledge(self):
        ...
        try:
            ...
            if not files:
                self.console.print(
                    "[yellow]このアシスタントにはナレッジが登録されていません。[/yellow]"
                )
                return
            ...
            sel = Prompt.ask(
                "削除するNo.を選択", choices=[str(i) for i in range(1, len(files) + 1)]
            )
            target = files[int(sel) - 1]
            file_uid = (
                target.get("uid")
                or target.get("id")
                or target.get("file_uid")
                or target.get("file_id")
            )
            if not file_uid:
                self.console.print(
                    "[red]選択した項目のファイルIDが取得できません。[/red]"
                )
                return
            ok = self.client.delete_assistant_knowledge(assistant_uid, str(file_uid))
            self.console.print(
                "[green]削除に成功しました。[/green]"
                if ok
                else "[red]削除に失敗しました。[/red]"
            )
        except Exception as e:
    ...
    def _show_settings(self):
        ...
        try:
            ...
            settings_table.add_row(
                "Company Subdomain", config.company_subdomain or "未設定"
            )
            ...
            auth_table.add_row(
                "認証済み", "はい" if self.auth_manager.is_authenticated() else "いいえ"
            )
            auth_table.add_row(
                "セッションCookie数", str(len(self.auth_manager.session_cookies))
            )
            auth_table.add_row(
                "トークン情報", "あり" if self.auth_manager.tokens else "なし"
            )
    ...
    def _show_available_audio_files(self):
        ...
        try:
            ...
            if audio_files:
                ...
                self.console.print(
                    "[yellow]現在のディレクトリに音声ファイルが見つかりません。[/yellow]\n"
                )
        except Exception as e:
            # エラーが発生しても処理を続行
            pass
