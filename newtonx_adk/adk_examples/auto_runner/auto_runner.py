...
class AutoRunner:
    ...
    def __init__(self, config_path: str):
        ...
        self.default_assistant_name: str = (self.config.get("chat") or {}).get(
            "default_assistant"
        ) or "高速アシスタント（GPT-4o mini）"
        # アップロード後の待機秒数（メッセージ送信前に待機）
        try:
            self.wait_after_upload_seconds: int = int(
                (self.config.get("chat") or {}).get("wait_after_upload_seconds", 0)
            )
        except Exception:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.console.print(
                f"[green]設定ファイルを読み込みました: {self.config_path}[/green]"
            )
            return config
            ...
            with Progress(
                ...
                for i, task in enumerate(enabled_tasks, 1):
                    task_id = progress.add_task(
                        f"[{i}/{len(enabled_tasks)}] {task['name']}", total=None
                    )
                    ...
                    try:
                        ...
                        progress.update(
                            task_id, description=f"[green]✓ {task['name']}[/green]"
                        )
                        ...
                        progress.update(
                            task_id, description=f"[red]✗ {task['name']}[/red]"
                        )
    ...
    def _authenticate(self) -> bool:
        ...
        try:
            ...
            if self.auth_manager.is_authenticated():
                ...
                try:
                    ...
                    if "Authorization" not in headers:
                        self.console.print(
                            "[yellow]Bearerトークンが見つからないため、内部トークンの貼り付けを促します。[/yellow]"
                        )
                        pasted = input(
                            "PATを貼り付けてください（WEBアプリで発行）: "
                        ).strip()
                        import re as _re
            ...
            if not ok:
                self.console.print(
                    "[yellow]ブートストラップに失敗しましたが続行します。[/yellow]"
                )
    ...
    def _task_upload(self, task: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if os.path.exists(file_path):
                ...
                if file_ext in [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp"]:
                    ...
                    result = self.client.upload_document(
                        self.current_chat_uid, file_path
                    )
                    uploaded_files.append(
                        {
                            "file": file_path,
                ...
                self.console.print(
                    f"  [yellow]ファイルが見つかりません: {file_path}[/yellow]"
                )
        ...
        for file_info in uploaded_files:
            if file_info.get("document_id"):
                self.logger.info(
                    f"ドキュメントアップロード成功: {file_info['file']} -> uid: {file_info['document_id']}"
                )
            elif file_info.get("image_id"):
                self.logger.info(
                    f"画像アップロード成功: {file_info['file']} -> uid: {file_info['image_id']}"
                )
            else:
                self.logger.warning(
                    f"ファイルアップロード成功だがuidが取得できません: {file_info['file']}"
                )
    ...
    def _task_send_message(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.uploaded_files and self.wait_after_upload_seconds > 0:
            try:
                self.console.print(
                    f"  [dim]{self.wait_after_upload_seconds}秒待機してから送信します（アップロード反映待ち）[/dim]"
                )
                import time as _t
