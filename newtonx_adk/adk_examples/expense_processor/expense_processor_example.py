...
class ExpenseProcessorADK:
    ...
    def _authenticate(self) -> bool:
        ...
        try:
            ...
            if is_auth:
                ...
                try:
                    ...
                    if "Authorization" not in headers:
                        self.logger.info(
                            "Bearerトークンが見つからないため、内部トークンの貼り付けを促します。"
                        )
                        pasted = input(
            ...
            if not config.client_id or not config.tenant_id:
                ...
                self.logger.error(
                    "以下のコマンドで認証設定を行ってください: python tools/setup_config.py"
                )
                return False
    ...
    def create_work_folder(self, folder_name: str) -> str:
        ...
        try:
            ...
            if folder_uid:
                self.logger.info(
                    f"作業フォルダを作成しました: {folder_name} (ID: {folder_uid})"
                )
                return folder_uid
            else:
        ...
        try:
            ...
            if file_extension in [".jpg", ".jpeg", ".png", ".gif"]:
                ...
                try:
                    ...
                    if image_id:
                        self.logger.info(
                            f"画像ファイルをアップロードしました: {file_path} (ID: {image_id})"
                        )
                        return image_id
                    else:
                        self.logger.error(
                            f"画像アップロードに失敗しました: {file_path}"
                        )
                        return None
                except Exception as e:
                ...
                if success:
                    self.logger.info(
                        f"ドキュメントファイルをアップロードしました: {file_path}"
                    )
                    return True
                else:
    ...
    def extract_expense_info(
        self, chat_uid: str, message_response: Optional[str]
    ) -> List[Dict]:
        """メッセージ応答から経費情報を抽出"""
        try:
            if message_response:
    ...
    def process_files(self, input_dir: str, output_file: str):
        ...
        try:
            ...
            for i, a in enumerate(assistants, 1):
                print(
                    f"  {i}. {a.get('name', '名前なし')} (UID: {a.get('uid', '不明')[:8]}...)"
                )
            print("=" * 35)
            ...
            while True:
                try:
                    choice = input(
                        f"\n使用するアシスタントの番号を入力してください (1-{len(assistants)}): "
                    ).strip()
                    choice_num = int(choice)
                    if 1 <= choice_num <= len(assistants):
                        assistant = assistants[choice_num - 1]
                        self.logger.info(
                            f"アシスタント '{assistant.get('name')}' を選択しました"
                        )
                        break
                    else:
                    ...
                    if expense_info:
                        if len(expense_info) == len(image_files):
                            ...
                            for info in expense_info:
                                ...
                                all_results.append(info)
                        self.logger.info(
                            f"画像から経費情報を抽出しました: {len(expense_info)}件"
                        )
                    else:
                ...
                try:
                    ...
                    if expense_info:
                        for info in expense_info:
                            ...
                            all_results.append(info)
                        self.logger.info(
                            f"ドキュメントから経費情報を抽出しました: {len(expense_info)}件"
                        )
                    else:
                        self.logger.warning(
                            f"ドキュメントの経費情報抽出に失敗: {doc_path}"
                        )
                except Exception as e:
            ...
            if all_results:
                ...
                self.logger.info(
                    f"処理完了: {len(all_results)}件の経費情報を{output_file}に出力しました"
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NewtonX ADKを使用した経費精算支援アプリケーション"
    )
