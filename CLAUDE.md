...
- **リリースは `main` へマージ後に `backend/release.bat` を実行する**（`main` への切り替え・最新化→テスト→リリース前スモーク〈起動・既存DBの移行確認〉→バージョン入力→パッケージ→タグ→**下書き**リリース作成まで自動。ローカルは dev ブランチのままでよい）。人の作業はGitHubの画面で本文を書き換えて公開するだけ。ノートを先に書く方式は `backend/scripts/release.py <version>`。工程を個別に叩くと順序違いが事故になる。詳細は `OPERATIONS.md` 7.4
