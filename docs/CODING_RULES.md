...
- **機械的に計測する**: バックエンドは `pytest --cov`、フロントエンドは `npm run test`（Vitest + v8）がいずれも閾値100%で失敗する。計測対象・除外範囲とその理由は `OPERATIONS.md`「フロントエンドのカバレッジ」を参照
- **フロントの分岐は `.ts` へ切り出す**: 描画基盤（jsdom / @testing-library）が無く `.tsx` はテストできないため、判定ロジックは純粋関数として `.ts` に置き、コンポーネントは描画と通信に専念させる
