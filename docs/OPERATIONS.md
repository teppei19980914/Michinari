...
テスト用DBの削除が`PermissionError: [WinError 32]`になる形の失敗も過去に起きているが
（`backend/tests/conftest.py`）、これはOneDriveロックとは別種の問題と判明している。
2026-09-14に再試行時間を1秒×5回→3秒×5回（最大15秒）へ延長、2026-09-16に固定パスを
`tempfile.gettempdir()`配下（OneDrive同期対象外）へ移したが、いずれも再発した
（OneDrive同期フォルダ外の`%TEMP%`でも同じ`PermissionError`が発生したため、
原因はOneDriveのファイルオンデマンドロックではなく、アンチウイルスの一時スキャン等
他プロセスによる一時的なロックと考えられる）。根本原因が「固定パスの前回実行の
残骸ファイルを、新しい実行が削除しようとして他プロセスのロックに阻まれる」という
設計自体にあると判断し、2026-09-16に`tempfile.TemporaryDirectory(ignore_cleanup_errors=True)`
で実行のたびに一意なディレクトリを割り当てる方式へ変更した（`scripts/release_smoke.py`の
スモークDBと同じ方針）。これにより起動時に削除すべき既存ファイルが存在しなくなり、
このクラスの失敗が起こり得なくなった。終了時の後片付けもエラーを無視するため、万一
ロックが残っていてもテスト結果には影響しない（再試行付き削除の仕組み`db_retry.py`は
不要になったため削除した）。
